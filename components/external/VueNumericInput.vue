...
export default {
  ...
  methods: {
    checkDecimals(event) {
      ...
      if (this.countDecimals(val) > this.decimals && this.decimals !== 0) {
        const splitted = val.toString().split(".");
        const digits = splitted[0];
        const decimals = splitted[1].slice(0, this.decimals);
        val = digits + "." + decimals;
        this.setValue(val);
        this.$notify({
          type: "error",
          title: `Can't be more than ${this.decimals} decimals`,
    ...
    decrease() {
      if (this.decreasable) {
        ...
        num = num.minus(+this.step).toNumber();
        if (+num > this.max) this.setValue(this.max);
        else if (+num < this.min) this.setValue(this.min);
        else this.setValue(num);
      }
    },
    ...
    increase() {
      if (this.increasable) {
        const val = this.currentValue;
        let num = new BigNumber(val);
        num = num.plus(+this.step).toNumber();
